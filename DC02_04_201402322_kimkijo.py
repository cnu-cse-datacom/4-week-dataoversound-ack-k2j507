...
def get_freq(chunk_bits, data_to_send):
    chunk = []
    for data in data_to_send:
        chunked_data = []
        for i in range(0,8//chunk_bits):
            shift = (chunk_bits*i)
            shifted_data = (data>>shift) & 0xf
            chunked_data.append(shifted_data)
        chunk.append(chunked_data[::-1])

    bitchunk = []
    for raw in chunk:
        bitchunk.append(raw[0])
        bitchunk.append(raw[1])

    freqs = [(i * STEP_HZ + START_HZ) for i in bitchunk]
    ...
    while True:
        ...
        if in_packet and match(dom, HANDSHAKE_END_HZ):
            ...
            try:
                ...
                if "201402322" in byte_stream:      
                    p = pyaudio.PyAudio()
                    stream = p.open(format=pyaudio.paFloat32, channels=1,
                                    rate=44100, output=True)
                    temp_byte_stream = byte_stream
                    temp = byte_stream.split("201402322 ")
                    byte_stream = ''.join(temp)

                    
                #print(byte_stream)
                print(temp_byte_stream) #print mic input
                print(byte_stream)
                #print(chunk)
                #print(temp) 
                display(byte_stream)
                
                encode_byte_stream = RSCodec(FEC_BYTES).encode(byte_stream)
                encode_byte_stream = byte_stream.encode("utf-8")
                
                print(encode_byte_stream)

            
                '''
                freq.append(HANDSHAKE_START_HZ)

                for i in range(1, len(byte_stream)):
                    temp_freq = encode_byte_stream[i] >> 4
                    print("good")
                    print(temp_freq)
                    freq.append((temp_freq * STEP_HZ) + START_HZ)

                freq.append(HANDSHAKE_END_HZ)
                '''
                freq =  get_freq(BITS, encode_byte_stream)
                ''' 
                p = pyaudio.PyAudio()
                stream = p.open(format=pyaudio.paFloat32, channels=1,
                                rate=44100, output=True)
                ...
                for j in range(1, len(freq)):
                    print(freq[j])
                    samples = (np.sin(2*np.pi*np.arange(44100 * DURATION)*(freq[j]/44100))).astype(np.float32).tobytes()
                    #print(samples)
